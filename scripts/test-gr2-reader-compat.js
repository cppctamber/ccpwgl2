...
function loadPreparation(formatGr2)
{
    const source = fs.readFileSync(path.resolve(__dirname, "../src/core/reader/geometry/Gr2Preparation.js"), "utf8")
        .replace(/^import .*;\r?\n/gm, "").replace(/^export /gm, "");
    return new Function("CjsGr2Format", source + "; return { prepareGr2, prepareGr2JSON, normalizeGr2Curve, normalizeGrannyKeys };")(formatGr2);
}
...
function loadReader(formatGr2)
{
    ...
    const modules = {
        "./Gr2Preparation": loadPreparation(formatGr2),
        "@carbonenginejs/runtime/resource/formats/gr2": { __esModule: true, default: formatGr2, CjsGr2Format: formatGr2 },
...
function testMorphsAndBindings(Gr2Reader)
{
    ...
    const prepared = loadPreparation({}).prepareGr2JSON(structuredClone(data), { firstMeshOnly: false });
    const bindCalls = [];
    const res = {
        meshes: [],
        models: [],
    ...
    const preparedRes = { meshes: [], models: [], animations: [], constructor: { BindMeshToModel() {} } };
    Gr2Reader.BuildGeometryRes(prepared, preparedRes, { firstMeshOnly: false });
    assert.deepEqual(preparedRes.meshes[0].bufferData, res.meshes[0].bufferData, "Worker-packed vertices match the existing builder");
    assert.deepEqual(preparedRes.meshes[0].indexData, res.meshes[0].indexData, "Worker-packed indices match the existing builder");
    assert.equal(preparedRes.meshes[0].declaration.stride, res.meshes[0].declaration.stride);
...
async function main()
{
    ...
    assert.equal(CjsGr2Format.isGsf(rawGsf), true);
    assert.equal(CjsGr2Format.readGsf(rawGsf).format, "gsf");
    assert.equal(CjsGr2Format.isGsf(new Uint8Array(16)), false, "Byte detection must fail closed for non-GSF data");
