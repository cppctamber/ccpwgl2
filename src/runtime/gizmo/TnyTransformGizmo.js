...
export class TnyTransformGizmo extends WglTransform
{
    ...
    /**
     * The client this gizmo belongs to, injected by TnyClient.Create so the
     * gizmo never has to import (and cycle back to) the exported client.
     * @type {null|TnyClient}
     */
    client = null;
     ...
     * @returns {TnyTransformGizmo}
     */
    EnableControls(target = this._installedGizmoScene || this.ResolveDefaultInstallTarget(), element)
    {
        const
            scene = this.ResolveControlScene(target),
            dom = element || this.ResolveControlElement(target),
            keyTarget = this.ResolveControlKeyTarget(target),
            camera = this.ResolveControlCamera(target);
        ...
        const input = this.ResolveControlInput(target);
     ...
     * @returns {TnyTransformGizmo}
     */
    Install(target = this.ResolveDefaultInstallTarget())
    {
     ...
     * @returns {*}
     */
    ResolveControlKeyTarget(target)
    {
        if (target)
        {
