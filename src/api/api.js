// Temporary API gateway home.
// This will grow ccpwgl-server specific routes first, then fall back to
// the basic public ESI/CDN style helpers when those routes are unavailable.
...
function withIDAliases(item, aliases)
{
    if (!item) return item;

    for (let i = 0; i < aliases.length; i++)
    {
        const [ src, dest ] = aliases[i];
        if (item[src] !== undefined && item[dest] === undefined)
        {
            item[dest] = item[src];
        }
    }
...
function createEsiCollection(route, aliases = [])
{
    return {
        async getOne(id, params)
        {
            const item = normalizeEsiObject(await getIDFromESIRoute(route, id, params));
            return withIDAliases(item, aliases);
        },

        async get(id, params)
        {
            if (id !== undefined && id !== null) return this.getOne(id, params);

            const url = buildESIUrlString(route, params);
            if (!cache.has(url))
            {
                cache.set(url, resMan.FetchRaw(url, "json"));
            }
...
export const categories = createEsiCollection(EsiUniverse.CATEGORIES, [
    [ "categoryId", "categoryID" ]
]);

export const groups = createEsiCollection(EsiUniverse.GROUPS, [
    [ "categoryId", "categoryID" ],
    [ "groupId", "groupID" ]
]);

export const types = createEsiCollection(EsiUniverse.TYPES, [
    [ "groupId", "groupID" ],
    [ "typeId", "typeID" ]
]);

export const graphics = createEsiCollection(EsiUniverse.GRAPHICS, [
    [ "graphicId", "graphicID" ]
]);

export const marketGroups = createEsiCollection("markets/groups", [
    [ "marketGroupId", "marketGroupID" ],
    [ "parentGroupId", "parentGroupID" ]
]);

export async function getCategoryID(categoryID, params)
{
    return categories.getOne(categoryID, params);
}

export async function getGroupID(groupID, params)
{
    return groups.getOne(groupID, params);
}
...
export async function getSkinMaterialID(materialTypeID)
{
    ...
    if (!cache.has(url)) cache.set(url, resMan.FetchRaw(url, "json"));
    return await cache.get(url);
}
...
export async function getSkinMaterialTypeIDs(skinMaterialID)
{
    ...
    if (!cache.has(url)) cache.set(url, resMan.FetchRaw(url, "json").catch(err => []));
    return await cache.get(url);
}
