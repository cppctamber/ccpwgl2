...
import { TEX_2D, RS_ZENABLE, RS_CULLMODE, CULL_CCW } from "constant/d3d";


/**
 * Hand crafted booster volumetric shader.
 *
 * The two programs are CCP's own GLES2 `boostervolumetric.sm_hi`, taken
 ...
 * around them, which is where the shipped container is wrong for us.
 *
 * That container types NoiseMap as a VOLUME, so the engine binds a
 * `TEXTURE_3D`. But these programs are ESSL1 and ESSL1 has no `sampler3D`, so
 * every WebGL build takes the shader's own fallback branch - it does
 * `#define sampler3D sampler2D` and reads the volume as a flat sheet of slices
 * addressed with the `s2sl` slice count. The shipped declaration therefore asks
 * for a 3D texture the compiled program cannot sample, which is why the booster
 * noise renders dead.
 *
 * NoiseMap is declared 2D here with `isVolume` kept true: 2D so the sheet binds,
 * `isVolume` so the slice count still reaches the shader. `Tw2TextureRes`
 * realizes the volume DDS into whichever of the two representations a sampler
 * asks for.
 */
...
export const boostervolumetric = {
    ...
    techniques: {
        Main: {
            vs: {
                // Container order, which is also attr0..attr6. The vertex
                // layout carries TEXCOORD0 and TEXCOORD5 but the program never
                // reads them, so they are absent here exactly as they are there.
                inputDefinitions: [
            ...
            ps: {
                ...
                constants: [
                    ...
                    colour("WarpColor0", "Warp flame colour"),
                    BoosterScale
                ],
                textures: [
                    ...
                    createTex("GradientMap0", TEX_2D, {
                        ...
                        ui: { description: "Booster gradient" }
                    }),
                    // 2D because an ESSL1 program cannot sample a sampler3D, but
                    // still flagged volume so the slice count reaches s2sl. It
            ...
            // at the wrong end of the booster.
            states: {
                [RS_ZENABLE]: 1,
