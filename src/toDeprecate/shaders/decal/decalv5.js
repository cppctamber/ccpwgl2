...
import { G_PIXEL_ADJUSTMENT } from "../shared/constant";
import {
    DecalDiffuseMode,
    DecalDiffuseColorModes,
    DecalDiffuseColorOverride,
    DecalChromaKeyColor,
    DecalChromaKeyData,
    DecalChromaKeyData2
} from "./shared/constant";
import { decalExtendedPickingV5 } from "./extended";
...
export const decalV5 = {
    ...
    techniques: {
        Main: {
            ...
            ps: {
                constants: [
                    // NONE OF THESE ARE STANDARD. SO NOT CCP SUPPORTED
                    G_PIXEL_ADJUSTMENT,
                    DecalDiffuseMode,
                    DecalDiffuseColorOverride,
                    DecalChromaKeyData,
                ...
                shader: `
                    ...
                    vec3 rgbToYCbCrLinear(vec3 value)
                    {
                        float luma = dot(value, vec3(0.299, 0.587, 0.114));
                        float blueDiff = (value.b - luma) * 0.564;
                        float redDiff  = (value.r - luma) * 0.713;
                        return vec3(luma, blueDiff, redDiff);
                    }

                    float chromaDistanceLinear(vec3 colorLin, vec3 keyLin)
                    {
                        vec3 delta = rgbToYCbCrLinear(colorLin) - rgbToYCbCrLinear(keyLin);
                        delta.x *= 0.20; // small luma weight
                        return length(delta);
                    }

                    float backgroundLikelihood(float distanceValue, float tolerance, float softness)
                    {
                        return 1.0 - smoothstep(tolerance, tolerance + max(softness, 1e-6), distanceValue);
                    }

                    float chromaConsensusAlpha(
                        ...
                        float invertFlag)
                    {
                        vec2 offset = vec2(uvStep, uvStep);

                        vec3 tap0 = clampToBorder(sourceTexture, uvCoords).xyz;
                        vec3 tap1 = clampToBorder(sourceTexture, uvCoords + vec2( offset.x, 0.0)).xyz;
                        vec3 tap2 = clampToBorder(sourceTexture, uvCoords + vec2(-offset.x, 0.0)).xyz;
                        vec3 tap3 = clampToBorder(sourceTexture, uvCoords + vec2(0.0,  offset.y)).xyz;
                        vec3 tap4 = clampToBorder(sourceTexture, uvCoords + vec2(0.0, -offset.y)).xyz;
                        vec3 tap5 = clampToBorder(sourceTexture, uvCoords + vec2( offset.x,  offset.y)).xyz;
                        vec3 tap6 = clampToBorder(sourceTexture, uvCoords + vec2(-offset.x,  offset.y)).xyz;
                        vec3 tap7 = clampToBorder(sourceTexture, uvCoords + vec2( offset.x, -offset.y)).xyz;
                        vec3 tap8 = clampToBorder(sourceTexture, uvCoords + vec2(-offset.x, -offset.y)).xyz;

                        float voteSum =
                            backgroundLikelihood(chromaDistanceLinear(tap0, keyColorLinear), tolerance, softness) +
                            backgroundLikelihood(chromaDistanceLinear(tap1, keyColorLinear), tolerance, softness) +
                            backgroundLikelihood(chromaDistanceLinear(tap2, keyColorLinear), tolerance, softness) +
                            backgroundLikelihood(chromaDistanceLinear(tap3, keyColorLinear), tolerance, softness) +
                            backgroundLikelihood(chromaDistanceLinear(tap4, keyColorLinear), tolerance, softness) +
                            backgroundLikelihood(chromaDistanceLinear(tap5, keyColorLinear), tolerance, softness) +
                            backgroundLikelihood(chromaDistanceLinear(tap6, keyColorLinear), tolerance, softness) +
                            backgroundLikelihood(chromaDistanceLinear(tap7, keyColorLinear), tolerance, softness) +
                            backgroundLikelihood(chromaDistanceLinear(tap8, keyColorLinear), tolerance, softness);

                        float isBackgroundRegion = step(majority * 9.0, voteSum);

                        float centerDistance = chromaDistanceLinear(tap0, keyColorLinear);
                        float edgeFeather = smoothstep(tolerance, tolerance + max(softness, 1e-6), centerDistance);

                        float alphaValue = mix(edgeFeather, 1.0, 1.0 - isBackgroundRegion);

                        alphaValue = mix(alphaValue, 1.0 - alphaValue, step(0.5, invertFlag));
                        alphaValue = clamp(alphaValue * alphaGain, 0.0, 1.0);

                        return alphaValue;
                    }

                    vec3 despillToBlackLinear(
                        vec3 colorLinear,
                        vec3 keyColorLinear,
                        float alphaValue,
                        float tolerance,
                        ...
                        float strength)
                    {
                        float distanceValue = chromaDistanceLinear(colorLinear, keyColorLinear);
                        float nearKey = 1.0 - smoothstep(tolerance, tolerance + max(softness, 1e-6), distanceValue);

                        float alphaClamped = clamp(alphaValue, 0.0, 1.0);
                        float edgeBand =
                            smoothstep(0.02, 0.25, alphaClamped) *
                            (1.0 - smoothstep(0.85, 0.98, alphaClamped));

                        float spillAmount = nearKey * edgeBand * strength;
                        return mix(colorLinear, vec3(0.0), spillAmount);
                    }
                    ...
                    void main()
                    {
                        ...
                        float alphaFromTexture = r6.w;
                        float alphaFinal = alphaFromTexture;
                        vec3 albedoLinear = clampToBorder(s3, r1.xy).xyz;

                        float chromaEnable = cb7[3].x;

                        if (chromaEnable > 0.0)
                        {
                            float tolerance       = cb7[3].y;
                            float softness        = cb7[3].z;
                            float majority        = cb7[3].w;

                            float uvStep          = cb7[4].x;
                            float alphaGain       = cb7[4].y;
                            float despillStrength = cb7[4].z;
                            float invertFlag      = cb7[4].w;

                            vec3 keyColorLinear   = cb7[5].xyz;

                            // Clamp inputs
                            tolerance       = max(tolerance, 0.0);
                            softness        = max(softness, 1e-6);
                            majority        = clamp(majority, 0.0, 1.0);
                            alphaGain       = max(alphaGain, 0.0);
                            despillStrength = clamp(despillStrength, 0.0, 1.0);

                            // If uvStep is 0, fall back to a safe default (since maxSize is gone)
                            if (uvStep <= 0.0)
                            {
                                uvStep = 1.0 / 1024.0;
                            }

                            float alphaFromChroma = chromaConsensusAlpha(
                                s3,
                                r1.xy,
                                uvStep,
                                keyColorLinear,
                            ...
                            alphaFinal = alphaFromTexture * alphaFromChroma;

                            albedoLinear = despillToBlackLinear(
                                albedoLinear,
                                keyColorLinear,
                                alphaFromChroma,
                                tolerance,
                        ...
                        // global contrast (linear space)
                        //float contrastAmount = cb7[0].x;
                        //vec3 contrastPivot = vec3(0.18);
                        //albedoLinear = (albedoLinear - contrastPivot) * (1.0 + contrastAmount) + contrastPivot;
                        albedoLinear = clamp(albedoLinear, 0.0, 1.0);

                        // IMPORTANT: keep r6.xyz linear.
                        // The shader tail already encodes to sRGB for output.
                        r6.xyz = albedoLinear;
                        ...
                        // Overrides the decal colour
                        int mode=int(cb7[1].x);
                        if (mode==${DecalDiffuseColorModes.ADD})
                        {
                            r6.xyz=r6.xyz+cb7[2].xyz*cb7[2].www;
                        }
                        else if (mode==${DecalDiffuseColorModes.SUBTRACT})
                        {
                            r6.xyz=r6.xyz-cb7[2].xyz*cb7[2].www;
                        }
                        else if (mode==${DecalDiffuseColorModes.MULTIPLY})
                        {
                            r6.xyz=r6.xyz*cb7[2].xyz*cb7[2].www;
                        }
                        else if (mode==${DecalDiffuseColorModes.DIVIDE})
                        {
                            r6.xyz=r6.xyz/(cb7[2].xyz*cb7[2].www);
                        }
                        else if (mode==${DecalDiffuseColorModes.REPLACE})
                        {
                            r6.xyz=cb7[2].xyz*cb7[2].www;
                        }
        ...
        ExtendedPicking: decalExtendedPickingV5.techniques.Main
    }
};
