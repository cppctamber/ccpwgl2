...
export const quadUtilityDetailV5 = {
    name: "quadUtilityDetailV5",
        ...
        Main: {
            ...
            ps: {
                ...
                shader: `
                    ...
                    uniform sampler2D s11;  // Detail1Map
                    ...
                    vec4 getDetailMap(vec2 uv, sampler2D detailMap, vec4 detailData)
                    {
                        ...
                        // DetailData.x=scale,
                        // DetailData.z=offset x
                        // DetailData.w=offset y
                        vec2 detailUV = detailData.xx*uv.xy+detailData.zw;
                        vec4 color = texture2D(detailMap, detailUV);

                        // DetailData.y=intensity
                        color.w = detailData.y*color.w;
                        return color;
                    }
                    ...
                    void main()
                    {
                        ...
                        // Detail Map 2 first
                        else if (mode==${Mode.DETAIL_2_MAP})
                        {
                            r0=getDetailMap(v0, s12, cb7[4]);
                            r0.xyz=r0.xyz*vec3(r0.w);
                            r0.w=1.0;

                        }
                        // Detail Map 1 second
                        else if (mode==${Mode.DETAIL_1_MAP})
                        {
                            r0=getDetailMap(v0, s11, cb7[3]);
                            r0.xyz=r0.xyz*vec3(r0.w);
                            r0.w=1.0;
                        }
                        // Detail Map 3 second
                        else if (mode==${Mode.DETAIL_3_MAP})
                        {
                            r0=getDetailMap(v0, s13, cb7[5]);
                            r0.xyz=r0.xyz*vec3(r0.w);
                            r0.w=1.0;
                        }
...
export const skinnedQuadUtilityDetailV5 = {
    name: "skinned_quadUtilityV5",
    description: `skinned ${quadUtilityDetailV5.description}`,
    techniques: {
        Main: {
            vs: skinnedQuadV5_PosBwtTexTanTex,
            ps: quadUtilityDetailV5.techniques.Main.ps
        }
    }
};