...
/**
 * Stores schemas
 * @type {WeakMap<Function|class, Tw2StagingSchema>}
 */
 ...
 * @property {*} cache
 */
export default class Tw2StagingSchema
{
    ...
    cache = null;
    props = {
        name: Tw2StagingSchema.Type.STRING
    };
    ...
    {
        let {type = Constructor.name, category = null, isStaging = true, isLeaf = false, props={}} = options;

        if (inherits)
        {
            const schema = Tw2StagingSchema.get(inherits);
    ...
    {
        this.set(Constructor, new Tw2StagingSchema(Constructor, func(this.Type), inherits));
    }
...
class ErrSchemaDefinition extends Tw2Error
{
    constructor(data)
    {
        super(data, "Cannot redefine %type=class% schema twice");
    }
}
