...
548 typed metadata records, 9,376 extension-neutral texture-metadata records,
...
`display: false`. The female foundation cannot use that strategy because its
head/body/feet are one monolithic body mesh. A metadata-qualified female boot
instead collapses only triangles influenced by the exact `LeftFoot`,
`RightFoot`, `LeftToe`, and `RightToe` binding prefixes. The mutation is
applied atomically at commit, leased across shared cached geometry, and
restores the captured indices after the final appearance releases. Live paper
doll `3000001` resolves `pantstuckshinshape` plus `bootmaskshin` as authored
shin-height evidence and reports 348 affected vertices and 618 affected
triangles. The mapping from authored height to the legacy foundation operation
is still demo policy; the dependency paths and their ordering are retained
library facts.
...
The inverse comparison is also visually disproved. Replacing only the exact
configured `C_Skin_blinn1` head carrier with the generic
`skinnedavatar.sm_hi` foundation shader makes the face extremely dark while
the body remains readable. That comparison switch was removed. The authored
head and rebuilt body carriers remain separate, and their remaining mismatch
must be resolved from retained material/composition inputs rather than by
substituting one shader contract for the other.
...
target. Paper doll `3000001` does not select an eyebrow colour. The exact
...
materially reduces the contract mismatch without dropping the face pipeline. Final
head/body colour parity still requires human visual qualification.
...
`comp_body_m.png`, and replaces RGB with the same selected shirt's exact
colourized detail, zones, and material values. Only the single ready demo-owned
fallback effect on `female/dependants/tuck/basic` receives the result. The
...
selected-top material and alpha transfer remain derived policy, and final
depth/coverage ownership remains open.

The initial committed demo revision retained authored depth testing and copied
RGB from the already-composed body atlas. The exact comparison remains
available as `?tuckRgb=body`; adding `&morphs=off` also reproduces the earlier
pre-deformation stage without deleting any retained morph request. Direct
visual review proves that checkpoint has correct torso colour as well as clean
coverage and depth; its only visible colour seam is the separate head/body
problem. It is a historical diagnostic, not the intended final deformation
policy.
...
The same foundation `CutMaskMap` target can contain more than one independently
qualified mask. Female paper doll `3004001` selects exact
`female/bottomouter/pantsmf01` version `v2`; its retained metadata explicitly
occludes `bottomtight`, and its selected colorize-layer alpha contains 554,651
fully opaque pixels, 3,054,442 transparent pixels, and a small antialiased
edge. Only after that configured pants part is render-ready, the composer uses
that authored alpha as an additional black-cut body mask. The live report
records owner `female-pantsmf01`, the exact `v2` texture path, normalized atlas
placement, and one attached foundation effect. This is deliberately not a
`bottomouter` rule: skirts and every other bottom silhouette keep their legs
unless their own source-backed coverage contract is separately qualified.
