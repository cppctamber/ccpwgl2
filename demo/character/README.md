...
| `accessories/glasses` | The observed female choices and reviewed male Exo choice resolve through source-shaped configured-accessory contracts. EyeImp's exact `head` L/Z/N/S tuple and the Citadel/Pilot/Exo `acc` tuples reconstruct private targets and remain separate from garment composition. A decoded transparent GlassShader area with the complete authored glass contract is partitioned from its opaque frame and receives the same private D/N/S tuple atomically while retaining authored glass parameters and pass state. Full-placement control maps may scale independently of pixel aspect; cropped maps still require exact target-aspect agreement. Focused live renderer checks completed without material deferrals; broader family review remains pending. |
| `accessories/masks` | The observed male cyberpunk mask supplies one exact baked head D/N/S tuple to one opaque private linear-BRDF consumer. The adapter reconstructs and binds those channels without a palette guess or asset-name rule. Its separately retained head cut mask is reported but not attached to the private consumer because the decoded effect's cut influence is zero. The focused live proof is visually coherent; shared-head cut realization remains unresolved. |
...
| `accessories/nose` | The observed nose ring is a decoded material-only accessory: it has no texture or colour inventory, but retains one private linear-BRDF effect, nonzero material-library identity, UV bounds, material curves, and an exact N·L lookup. The adapter now completes only its missing samplers with neutral proof resources and promotes that retained compatible effect atomically. The live review renders the ring without diagnostic magenta; other shader/material topologies remain deferred. |
